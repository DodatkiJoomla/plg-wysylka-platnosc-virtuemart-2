...
class plgSystemWysylka_platnosc_vm2 extends JPlugin
{
    ...
    {
        if (isset($_REQUEST['option']) && $_REQUEST['option'] == 'com_virtuemart' && isset($_REQUEST['view']) && $_REQUEST['view'] == 'cart' && isset($_REQUEST['task']) && $_REQUEST['task'] == 'editpayment') {
            // koszyk
            ...
            if (!$id_wys) {
                ...
                if ($this->czyPuste() != false) {
                    $params = & JComponentHelper::getParams('com_wysylka_platnosc_vm2');
                    //var_dump($params);
                    // pola pluginu
                    for ($i = 1; $i <= 50; ++$i) {
                        ...
                        if ($params->get("shipment_name" . $i) == $id_wys) {
                            ...
                            $id_platnosci = $params->getValue('payment_name' . $i);
    ...
    function czyPuste()
    {
        ...
        $tab = array();
        for ($i = 1; $i <= 50; ++$i) {
            if ($params->getValue('shipment_name' . $i)) {
                $tab[] = $params->getValue('shipment_name' . $i);
            }
        }
        if (count($tab) == 0) {
            return false;
